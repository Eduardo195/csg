...
const Connector = {
  ...
  getDistricts() {
    return this.connect().then(db => {
      return db.collection(TABLE_DISTRICTS).find().sort({index: 1}).toArray().then(data => {
        db.close();
        return data;
      });
    });
  ...
  getContractTypes() {
    return this.connect().then(db => {
      return db.collection(TABLE_CONTRACT_TYPES).find().sort({index: 1}).toArray().then(data => {
        db.close();
        return data;
      });
    });
  ...
  getByRef(ref){
    return this.connect().then(db => {
      return db.collection(TABLE_NAME).find({ _id: ObjectId(ref) }).toArray().then(data => {
        db.close();
        return data[0];
      });
    });
  ...
  latest({limit, page}) {
    ...
    return this.connect().then(db => {
      ...
      return find.count().then(count => {
        return find.skip(params.page * params.limit).limit(params.limit).toArray().then(data => {
          db.close();
          return {data, count};
        });
      });
